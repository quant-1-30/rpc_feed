...
class DuckDBManager: # 非线程安全
    ...
    async def query(self, req_meta: dict, raw_template: str):
        ...
        try:
            ...
            # extend ast depth from default 1000 to 2000 temporarily
            conn.execute("SET max_expression_depth = 2000;")
            # df = conn.execute(req_sql).df() / rows = conn.execute(req_sql).fetchmany(batch_size)
            reader = conn.execute(req_sql).fetch_record_batch(self.batch_size) # RecordBatchReader ChunkedArray and memory continual

            while True:
                try:
                    batch = reader.read_next_batch() # multi_columns bytes 
                    yield batch
